	...
	<div class="product product-{$product->id}">
    	<div class="product-image">
        	<a href="{$link->getProductLink($product)}" title="{$product->name}">
        		<img class="img-responsive" src="{$link->getImageLink($product->link_rewrite, $image, 'tm_small_default')|escape:'html':'UTF-8'}" alt="{$product->name|escape:'html':'UTF-8'}" />
            </a>
        </div>
        ...
        <p class="product-description">
        	{if $product->description_short|strip_tags:'UTF-8'|truncate:30:'...'}{$product->description_short|strip_tags:'UTF-8'|truncate:30:'...'}{/if}
        </p>
