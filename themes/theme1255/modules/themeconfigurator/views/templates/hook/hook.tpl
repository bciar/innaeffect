    ...
    <div id="htmlcontent_{$hookName}">
        <ul class="htmlcontent-home clearfix row">
            {foreach name=items from=$htmlitems item=hItem}
                <li class="htmlcontent-item-{$smarty.foreach.items.iteration|escape:'htmlall':'UTF-8'}{if $hookName == 'top'} col-xs-4{elseif $hookName =='footer'  || $hookName =='home'} col-xs-4{/if}">
                    {if $hItem.url}
